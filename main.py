...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.horizontalLayoutWidget_5.setGeometry(QtCore.QRect(210, 290, 1030, 580))
        ...
        self.org_textbrowser = QtWidgets.QTextBrowser(self.horizontalLayoutWidget_5)
        self.org_textbrowser.setObjectName("org_textbrowser")
        self.verticalLayout_5.addWidget(self.org_textbrowser)
        ...
        self.mod_textbrowser = QtWidgets.QTextBrowser(self.horizontalLayoutWidget_5)
        self.mod_textbrowser.setObjectName("mod_textbrowser")
        self.verticalLayout_7.addWidget(self.mod_textbrowser)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_5.setText(_translate("MainWindow", "Original File"))
        self.label_7.setText(_translate("MainWindow", "Modded File"))
        self.label_4.setText(_translate("MainWindow", "Choose File"))

    def openOrgAPK(self):
        options = QtWidgets.QFileDialog.Options()
        options |= QtWidgets.QFileDialog.DontUseNativeDialog
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Open File", "", "APK Files (*.apk)", options=options)
        if fileName:
    ...
    def onExtract(self):
        main_logic.extract(self.org_apkname, True, False)
        main_logic.extract(self.mod_apkname, True, True)

    def onDecompile(self):
        main_logic.decompile(self.mainfolder)
        for root, dirs, files in os.walk(os.path.relpath("diff/org/java")):
            for file in files:
                fullpath = os.path.join(root, file)
                self.diff_files.update({file: fullpath})
                item = QtWidgets.QListWidgetItem(file)
                self.diff_listview.addItem(item)

    def onDiff(self):
        self.diff_files.clear()
        main_logic.compare(self.min_byte_spinner.value(), self.mainfolder)
        main_logic.copy_diffs(self.mainfolder)

    def onDiffList(self, item):
        file = QtCore.QFile(self.diff_files.get(item.text()))
        if not file.open(QtCore.QIODevice.ReadOnly):
            QtGui.QMessageBox.information(None, 'info', file.errorString())
        stream = QtCore.QTextStream(file)
        self.org_textbrowser.setText(stream.readAll())

        file = QtCore.QFile(self.diff_files.get(item.text()).replace('/org/','/mod/'))
        if not file.open(QtCore.QIODevice.ReadOnly):
            QtGui.QMessageBox.information(None, 'info', file.errorString())
        stream = QtCore.QTextStream(file)
        self.mod_textbrowser.setText(stream.readAll())
